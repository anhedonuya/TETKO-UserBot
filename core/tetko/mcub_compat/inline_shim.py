...
class InlineHandlers:
    ...
    def create_inline_form(self, text, buttons=None, ttl=3600, media=None, media_type="photo"):
        ...
        try:
            ...
            if cache is not None:
                cache.set(form_id, {"text": text, "buttons": buttons,
                                    "media": media, "media_type": media_type}, ttl=ttl)
        except Exception:
            pass
        return form_id
    ...
    def get_inline_form(self, form_id):
        cache = getattr(self.kernel, "cache", None)
        return cache.get(form_id) if cache else None

    async def register_handlers(self):
        log.debug("InlineHandlers.register_handlers — заглушка")
