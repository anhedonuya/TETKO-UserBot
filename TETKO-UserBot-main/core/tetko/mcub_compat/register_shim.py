...
class RegisterShim:
    ...
    def bot_command(self, pattern, **kwargs):
        def decorator(fn):
            ...
            # Register on the bot client when one is available.
            client = getattr(self.kernel, "bot_client", None)
            if client is not None:
                try:
                    from telethon import events as tg_events
                    import re as _re
                    pat = kwargs.get("pattern") or rf"^/{_re.escape(name)}(?:\\s|$)"
                    async def _bot_wrapper(event):
                        return await self._invoke_handler(fn, event)
                    client.add_event_handler(_bot_wrapper, tg_events.NewMessage(pattern=pat))
                    self._events.append((_bot_wrapper, None, client))
                except Exception as e:
                    log.debug("[%s] bot command handler: %s", self.module_name, e)
            return fn
        return decorator

    def callback(self, func=None, *, ttl=900):
        """Register an inline callback method and expose a reusable token button."""
        def decorator(fn):
            self._callbacks = getattr(self, "_callbacks", {})
            self._callbacks[getattr(fn, "__name__", str(fn))] = fn
            # Keep callback metadata for introspection. Actual buttons are stored
            # in kernel.inline_callback_map by ButtonFactory.
            return fn
        ...
        def decorator(fn):
            ...
            try:
                ...
                event_map = {
                    ...
                    "messagedeleted": tg_events.MessageDeleted, "deleted": tg_events.MessageDeleted,
                    "messageread": tg_events.MessageRead, "read": tg_events.MessageRead,
                    "userupdate": tg_events.UserUpdate, "user": tg_events.UserUpdate,
                    "joinrequest": tg_events.JoinRequest, "request": tg_events.JoinRequest,
                    "album": tg_events.Album,
                    "raw": tg_events.Raw, "custom": tg_events.Raw,
                }
    ...
    def inline_temp(self, func, ttl=300, article=None, data=None,
                    allow_user=None, allow_ttl=100, **kwargs):
        import uuid
        temp_id = uuid.uuid4().hex
        self._inline_temp_map[temp_id] = {
            "handler": func,
            "article": article,
            "data": data,
    ...
    def cleanup_inline_temp(self):
        now = time.time()
        expired = [k for k,v in self._inline_temp_map.items()
                   if v.get("expires_at") and v["expires_at"] < now]
        for k in expired:
            self._inline_temp_map.pop(k, None)
        return len(expired)
    ...
    def get_command(self, command):
        ...
        return {"handler": self._commands.get(name),
                "owner": self.module_name if name in self._commands else None,
                "docs": {},
    ...
    def cleanup(self):
        """Remove Telethon event handlers and stop all background work."""
        self.stop_all_loops()
        for entry in list(self._events):
            try:
                handler, event_obj, client = entry
