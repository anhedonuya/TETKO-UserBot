...
def install_callback_handler(kernel, bot_client) -> None:
    ...
    async def _mcub_cb_handler(event):
        try:
            ...
            # Optional per-token user restriction.
            allowed_user = entry.get("allowed_user")
            allowed_until = entry.get("allow_expires_at")
            if allowed_user is not None:
                sender_id = getattr(event, "sender_id", None)
                if sender_id is None or int(sender_id) != int(allowed_user) or (
                    allowed_until and allowed_until <= time.time()
                ):
                        try:
            ...
            # first acknowledge Telegram's callback spinner, then invoke handler
            try:
                await event.answer()
            except Exception:
                pass
