...
def _install_fakes() -> None:
    """Install the MCUB compatibility import bridge.

    The metadata decorators from ``core.lib.loader.module_base`` are preserved;
    the compat ModuleBase consumes their ``_mcub_*`` attributes.
    """
    try:
        from .fake_package import install_fakes
        install_fakes()
    except Exception as _e:
        log.debug("[mcub_compat] import bridge: %s", _e)
    ...
    # Decorators are provided by the bundled MCUB-compatible module_base.
...
async def load_mcub_module(
    ...
    if not getattr(tetko_kernel, "_mcub_subsystems_installed", False):
        ...
        try:
            ...
            _client = getattr(tetko_kernel, "client", None)
            if _bot is not None:
                install_callback_handler(tetko_kernel, _bot)
            if _client is not None:
                install_callback_handler(tetko_kernel, _client)
        except Exception as e:
