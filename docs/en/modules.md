...
> [!CAUTION]
> `@callback` is registered but not yet hooked into Telethon's CallbackQuery events. Coming soon.
...
## 10. What NOT to do
  ...
  - Don't `import from core.lib.*`, `core.kernel.*`, `core.native.*` — they no longer exist.
  - Don't use `core_inline.*`, `utils.strings.Strings`, `core.langpacks` — not wired into the new kernel.
