...
> [!CAUTION]
> `@callback` пока регистрируется, но не подключён к событиям Telethon. Будет позже.
...
## 10. Чего НЕ делать
  ...
  - Не импортировать из `core.lib.*`, `core.kernel.*`, `core.native.*` — **их больше нет**.
  - Не использовать `core_inline.*`, `utils.strings.Strings`, `core.langpacks` — **не подключены**.
