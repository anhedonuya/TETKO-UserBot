...
class Backup(ModuleBase):
    ...
    async def create_backup_archive(
        ...
        backup_dir = temp_dir / "TETKO_backup"
        ...
        if archive_format == "tar.gz":
            archive_name = f"TETKO_backup{comp_suffix}_{timestamp}.tar.gz"
            ...
            archive_name = f"TETKO_backup{comp_suffix}_{timestamp}.zip"
            archive_path = temp_dir / archive_name
            compress_type = (
    ...
    async def _upload_yadisk(self, file_path: Path, token: str) -> bool:
        ...
        remote_path = f"/TETKO_Backups/{filename}"
        headers = {"Authorization": f"OAuth {token}"}

        async with aiohttp.ClientSession(headers=headers) as session:
            await session.put(
                "https://cloud-api.yandex.net/v1/disk/resources",
                params={"path": "/TETKO_Backups"},
            )
    ...
    async def _upload_gdrive(self, file_path: Path, token: str) -> bool:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            query = "name='TETKO_Backups' and mimeType='application/vnd.google-apps.folder' and trashed=false"
            list_data = await (
                await session.get(
    ...
    async def rotate_old_backups(self, chat_id: int, max_count: int) -> int:
        ...
        async for msg in self.client.iter_messages(chat_id, limit=500):
            if msg.document and msg.file and msg.file.name:
                name = msg.file.name
                if name.startswith("TETKO_backup") and (
                    name.endswith(".zip")
    ...
    async def ensure_backup_chat(self):
        ...
        try:
            result = await self.client(
                CreateChannelRequest(
                    title="TETKO Backups",
                    about="Automatic TETKO backups storage",
                    megagroup=True,
                )
            )
    ...
    async def _restore_from_backup_message(
        ...
        is_valid = fname.startswith("TETKO_backup") and (
        ...
        try:
            ...
            backup_dir = extract_dir / "TETKO_backup"
