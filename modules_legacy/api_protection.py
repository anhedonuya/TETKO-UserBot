...
DEFAULT_CONFIG = {
    ...
    # Telethon-TETKO native protection
...
def register(kernel):
    ...
    config = ModuleConfig(
        ...
        ConfigValue(
            "mcub_mode",
            "safe",
            description="TETKO protection mode",
    ...
    async def startup():
        ...
        # Apply TETKO protection mode now that config is loaded from DB
        if _mcub_available:
            ...
            if _apply_mcub_mode(mode):
                ...
                kernel.logger.info(
                    f"TETKO protection mode set to '{mode}' "
    ...
    def _apply_mcub_mode(mode: str) -> bool:
        """
        Apply a protection mode to the TETKO client.
        Returns True on success, False if TETKO is not available.
        """
        ...
        try:
            ...
            kernel.logger.error(f"Failed to apply TETKO mode '{mode}': {e}")
            return False

    async def _mcub_violation_handler(violation) -> None:
        """
        Called by TETKO on every blocked (or dry-run observed) request.
        ...
        try:
            ...
            kernel.logger.warning(f"TETKO violation [{mode}]: {method} - {reason}")
            ...
            kernel.logger.error(f"TETKO violation handler error: {e}")
    ...
    async def install_interceptor(kernel):
        ...
        if not _mcub_available:
            kernel.logger.debug(
                "Telethon-TETKO not detected, skipping native protection setup"
            )
    ...
    async def uninstall_interceptor(kernel):
        ...
        # Remove TETKO violation handler and reset to safe mode
        if _mcub_available:
            try:
                client.clear_blocked_request_handler()
                client.set_protection_mode("safe")
                kernel.logger.info("TETKO protection reset to 'safe' on uninstall")
            except Exception as e:
                kernel.logger.warning(f"TETKO cleanup error: {e}")
    ...
    async def api_protection_callback_handler(event):
        ...
        choose_mode_label = lang.get("mcub_choose_mode", "Choose TETKO protection mode:")
        default_mode_tpl = lang.get("mcub_mode_default", "Default ({mode})")
        mode_set_tpl = lang.get("mcub_mode_set", "TETKO protection mode: {mode}{dry}")
        ...
        if data == b"api_protection_yes":
            if _mcub_available:
                ...
                # No TETKO - just enable, no mode step
                protection_enabled = api_config["enable_protection"] = True
                await event.edit(
